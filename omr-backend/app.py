...
def export_excel():
    ...
    try:
        ...
        for idx, s in enumerate(students, start=1):
            ...
            verify = s.verify_ans if isinstance(s.verify_ans, dict) else {}
            ...
            for i in range(1, 41):
                ...
                v = verify.get(q_key, {}) if isinstance(verify.get(q_key), dict) else {}
            ...
            level_key = (s.level or "").strip().lower().replace(" ", "_")
            try:
                rules_db = ScoringRule.query.filter_by(level=level_key).order_by(ScoringRule.range_from).all()
                if rules_db:
                    max_marks = sum(float(r.correct_marks) for r in rules_db for _ in range(r.range_from, r.range_to + 1))
                else:
                    max_marks = 40.0
            except Exception:
                max_marks = 40.0
